...
public class ManageDepartmentsController implements Initializable {
    /******************************************************************************
     * FXML                                                                       *
     ******************************************************************************/
    @FXML
     ...
     * Observables                                                                *
     ******************************************************************************/
    ObservableList<BasicIdName> departments = FXCollections.observableArrayList();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        tblDepartments.setItems(departments);
    ...
    public void handleCategoryCellClicked(MouseEvent mouseEvent) {
        selectedDepartment = tblDepartments.getSelectionModel().getSelectedItem();
        ...
        if(mouseEvent.getClickCount() == 2) {
            AddViewDepartmentController controller = new AddViewDepartmentController();
            Scene scene = SceneManager.createScene(controller, Strings.FXML_SETTINGS_ADD_VIEW_CATEGORY);
            controller.setCallback(() -> {
                JDBC.openConnection();
                departments.clear();
                departments.addAll(QueryManager.getDepartments());
                JDBC.closeConnection();
                btnDelete.setDisable(true);
            });
            controller.populateData(selectedDepartment);
            SceneManager.showSceneInNewWindow( scene, Strings.TITLE_SETTINGS_EDIT_DEPARTMENT);
        }
    }
    ...
    public void handleNewCategory(ActionEvent actionEvent) {
        ...
        SceneManager.showSceneInNewWindow( scene, Strings.TITLE_SETTINGS_ADD_DEPARTMENT);
    }
    ...
    public void handleDeleteCategory(ActionEvent actionEvent) {
        JDBC.openConnection();
        QueryManager.deleteDepartment(selectedDepartment.getId());
        departments.clear();
        departments.addAll(QueryManager.getDepartments());
        JDBC.closeConnection();
        btnDelete.setDisable(true);
